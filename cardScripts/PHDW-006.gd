...
func resoForSkill(source,targetArray):
	var damageResult = await RulesEngine.dealDamageToZone(targetArray[0]["zoneID"], source, get_parent().myAttack)
	if damageResult == "thisKilledSomething":
		...
		if RulesEngine.spSkillChoiceZoneID != "FIZZLE":
			await RulesEngine.dealDamageToZone(RulesEngine.spSkillChoiceZoneID, source, get_parent().myAttack)
